...
# --- LỚP 1: DDOS TỔNG LỰC ---
    ...
    global_traffic['unique_ips'].add(src_ip) # Ghi nhận IP này vào danh sách
    
    if current_time - global_traffic['start_time'] >= 1.0:
        # CHỈ CẢNH BÁO MULTIPLE_IPS NẾU: Tổng gói > 500 VÀ số lượng IP tham gia phải lớn hơn 10
        if global_traffic['total_pkts'] > 500 and len(global_traffic['unique_ips']) > 10: 
            ts_str = time.strftime("%Y-%m-%d %H:%M:%S")
            ip_count = len(global_traffic['unique_ips'])
            gui_queue.put((ts_str, f"Nhiều IP ({ip_count} IPs)", "🚨 DDOS", global_traffic['total_pkts']))
            log_attack(ts_str, f"MULTIPLE_IPS ({ip_count})", "🚨 DDOS", global_traffic['total_pkts'])
            
        # Reset lại dữ liệu cho giây tiếp theo
        global_traffic['start_time'] = current_time
        global_traffic['total_pkts'] = 0
        global_traffic['unique_ips'].clear() # Xóa sạch danh sách IP cũ

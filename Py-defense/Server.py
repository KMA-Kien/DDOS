...
# 3. ==== Logic Bắt và Phân tích Gói tin ====
traffic_stats = {}
...
def analyze_traffic(ip, duration, stats):
    ...
    if stats['syn_pkts'] > 50: 
        print(f"🔴 PHÁT HIỆN TẤN CÔNG SYN FLOOD! (Phát hiện {stats['syn_pkts']} gói SYN)")
        prediction = 1
        label_detail = "🔥 SYN FLOOD"
    elif stats['src_pkts'] > 100:
        print("🔴 PHÁT HIỆN TẤN CÔNG DOS/FLOOD CHUNG (Lưu lượng bất thường)")
        prediction = 1
        label_detail = "🔥 DOS/FLOOD"
    else:
        if model:
            feature_names = [
...
SERVER_IP = "192.168.214.155" #khai báo ip
